...
describe("HealthServiceAreaLookup.vue", () => {
  it("latitude is required", async () => {
    ...
    const inputWrapper = wrapper.find("[data-testid=latitude]");
  ...
  it("longitude is required", async () => {
    const wrapper = factory();
    const inputWrapper = wrapper.find("[data-testid=longitude]");
    inputWrapper.setValue("");
    // waits for vue to validate the new value
    await Vue.nextTick();
    await Vue.nextTick();
  ...
  it("error message is displayed", async () => {
    ...
    await Vue.nextTick();
    await Vue.nextTick();
    const errorWrapper = wrapper.find('[data-testid="errorMessage"]');
    const resultWrapper = wrapper.find('[data-testid="successMessage"]');
